...
impl App {
    ...
    pub fn draw_modal_grep(&mut self, frame: &mut Frame) {
        ...
        let lines: Vec<Line> = vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled(if self.grep_editor.mode == EditorMode::Normal {"(enter)".to_string()} else { "enter".to_string() }, Style::default().fg(if self.grep_editor.mode == EditorMode::Normal { self.theme.COLOR_GREY_500 } else { self.theme.COLOR_GREY_600 })),
            ]),
        ];
        ...
        let modal_block = Block::default()
            ...
            .title(Span::styled(if self.grep_editor.mode == EditorMode::Normal {" (esc) "} else { "─ esc ─" }, Style::default().fg(if self.grep_editor.mode == EditorMode::Normal { self.theme.COLOR_GREY_500 } else { self.theme.COLOR_GREY_600 })))
        ...
        let editor_view = EditorView::new(&mut self.grep_editor).theme(custom_theme);
